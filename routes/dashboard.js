...
router.get('/', async function (req, res, next) {
    ...
    //filter orders by daysOn
    let daysOnfilter = [{id:'более 24 дней',count:0},{id:'более 14 дней',count:0},{id:'более 7 дней',count:0},{id:'от 0 дней',count:0}];
    orderslist.forEach((d) => {
        if(parseInt(d.daysOn) >= 24) daysOnfilter[0].count++;   
        if(parseInt(d.daysOn) >= 14) daysOnfilter[1].count++;   
        if(parseInt(d.daysOn) >= 7) daysOnfilter[2].count++;   
        if(parseInt(d.daysOn) >= 0) daysOnfilter[3].count++;   
    })

    res.render('dashboard', {
        title: 'Панель диаграмм',
        sidebar: 'filter',
        datenow: datenow,
        daysOn: daysOn,
